...
contract PreSaleVesting is Ownable {
  ...
  IERC20 private SaleToken = IERC20(0xb35516A60EED8600CAe11590f9957881B67f04a6);
...
uint256[] referLevels = [13,8,4,3,2];
  ...
  function buyTokens(uint256 quantity, address _referralAddress) external {
    require(currentPhase < 3, "Sale Has ended");
    require(quantity > 0, "Quantity should be more than 0");
    uint256 amount = (quantity * getCurrentPrice()) / 1e18;
    ...
    require(
      tokenSold + quantity <= phaseLimit[currentPhase],
      "Total supply should be greater "
    );
    require(
      tokenSold + quantity <= totalSupply,
      "Total supply should be greater "
    );
    ...
    uint256 _remainingTokens=  payReferRew(_referralAddress, amount);
    ...
    if(tokenSold == phaseLimit[2]){
      _vestingStatus=true;
      initialVestingDate = block.timestamp+ 35 days; //Change time here to change presale
    }
  }
...
if(uplineAddress == address(0)){
  ...
  function claimTokensS1() public {
    ...
    uint256 tokensToClaim = (sale.totalAmount * 20) / 100;
...
function getNextClaimS1(address _userAddress) public  view returns(uint256 ) {
 Sale1 storage sale = sale1[_userAddress];
   if(sale.totalAmount  == 0) return 0;
    uint256 tokensToClaim = (sale.totalAmount * 20 )/
      100;
 uint256 remainingTokens = sale.totalAmount-sale.claimed;
    if(remainingTokens ==0) return 0;
    if(remainingTokens < tokensToClaim){
